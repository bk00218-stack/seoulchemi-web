...
### 변경사항
1. **안경원명 클릭 → 수정 모달 열기**
   - 기존: Link로 `/stores/[id]` 상세 페이지 이동 (API 500 에러로 동작 안 함)
   - 변경: span + onClick으로 수정 모달 직접 열기
   - 파란색 제거 (일반 텍스트처럼)

2. **검색필터 테두리 연하게**
   - 기존: `border: 2px solid #007aff` (진한 파란색)
   - 변경: `border: 1px solid #c5d9f1` (연한 파란색)
   - hover borderColor: `#007aff` → `#86b7fe`

3. **API 에러 디버깅 개선**
   - `/api/stores/[id]` GET에서 에러 발생 시 details 포함하도록 수정

### 커밋
- `feat: 안경원명 클릭 시 수정 모달 열기`
- `style: 안경원명 파란색 제거, 검색필터 테두리 연하게`
- `chore: add error details to store API for debugging`

### 배포
- Vercel Production 완료

### 참고
- `/stores/[id]` 상세 페이지 API가 500 에러 발생 중 (나중에 수정 필요)
- 원인: `prisma.store.findUnique()` 관련 에러 추정
