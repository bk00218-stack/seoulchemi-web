...
// 콘택트렌즈 브랜드
const CONTACT_BRANDS = ['아큐브', '알콘', '바슈롬', '쿠퍼비젼', '클라렌', 'I LENS', '인터로조'];

function isContact(brandName) {
  return CONTACT_BRANDS.some(cb => brandName.includes(cb));
}
...
async function main() {
  ...
  console.log('=== 콘택트렌즈 상품 임포트 ===\n');

  // 1. 기존 콘택트 상품 비활성화
  console.log('1. 기존 콘택트 상품 비활성화...');
  ...
  for (let i = 7; i < data.length; i++) {
    ...
    if (!brand || !isContact(brand)) continue;

    productRows.push({
      brand,
      name: String(row[2] || '').trim(),
      purchasePrice: Math.round(Number(row[13]) || 0),
      sellingPrice: Math.round(Number(row[14]) || 0),
      retailPrice: Math.round(Number(row[16]) || 0),
    });
  }
  console.log(`   콘택트 상품: ${productRows.length}개\n`);
  ...
  for (const brandName of uniqueBrands) {
    let existing = await prisma.brand.findFirst({
      where: { name: brandName, categoryId: 3 }
    });
  ...
  console.log(`   총 ${uniqueBrands.length}개 브랜드\n`);
  ...
  const totalActive = await prisma.product.count({
    where: { brand: { categoryId: 3 }, isActive: true }
  });
  const totalBrandsActive = await prisma.brand.count({
    where: { categoryId: 3, isActive: true }
  });
  console.log(`\n현재 콘택트 활성 브랜드: ${totalBrandsActive}개`);
  console.log(`현재 콘택트 활성 상품: ${totalActive}개`);
}
