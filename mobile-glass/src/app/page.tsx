  ...
  return (
    <Layout sidebarMenus={SIDEBAR} activeNav="주문">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
        {[
          { label: '오늘 주문', value: stats.totalOrders, unit: '건', color: 'var(--gray-700)' },
          { label: '대기중', value: stats.pendingOrders, unit: '건', color: 'var(--warning)', bg: 'var(--warning-light)' },
          { label: '출고완료', value: stats.completedOrders, unit: '건', color: 'var(--success)', bg: 'var(--success-light)' },
          { label: '총 매출', value: stats.totalRevenue.toLocaleString(), unit: '원', color: 'var(--primary)', bg: 'var(--primary-light)' },
        ].map((stat, i) => (
          <div key={i} style={{
            ...
            <div style={{ fontSize: 13, color: 'var(--gray-500)', marginBottom: 8 }}>{stat.label}</div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {['주문', '반품', '전체'].map((t, i) => (
            <label key={t} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 13, cursor: 'pointer' }}>
              <input type="radio" name="type" defaultChecked={i === 2} style={{ accentColor: 'var(--primary)' }} /> {t}
            </label>
          ))}
        </div>
        ...
        <div style={{ flex: 1 }} />
        <button style={{ ...btnStyle, background: 'var(--success)', color: '#fff', border: 'none' }}>📥 엑셀 다운</button>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            <span style={{ fontSize: 15, fontWeight: 600 }}>주문 목록</span>
        ...
        <div style={{ overflow: 'auto', flex: 1 }}>
          <table style={{ width: '100%', minWidth: 1000 }}>
            <thead>
              <tr>
                <th style={thStyle}>
                  <input type="checkbox" onChange={toggleSelectAll} checked={selectedIds.size === orders.length && orders.length > 0} style={{ accentColor: 'var(--primary)' }} />
                </th>
                <th style={thStyle}>#</th>
              ...
              {rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr key={`${row.order.id}-${row.item?.id || 0}-${row.idx}`}>
                    ...
                    <td style={{ ...tdStyle, color: 'var(--gray-400)', fontSize: 12 }}>{row.idx}</td>
                    {row.isFirst && (
                      <td style={tdStyle} rowSpan={row.span}>
                        <span style={{ color: 'var(--primary)', fontWeight: 500 }}>{row.order.orderNo}</span>
                      </td>
                    )}
                    {row.isFirst && (
                    ...
                    {row.isFirst && (
                      <td style={{ ...tdStyle, fontSize: 12, color: 'var(--gray-500)' }} rowSpan={row.span}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <button style={{ ...btnStyle, color: 'var(--danger)' }}>선택 삭제</button>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button style={{ ...btnStyle, background: 'var(--primary)', color: '#fff', border: 'none' }}>발송준비</button>
            <button style={{ ...btnStyle, background: 'var(--success)', color: '#fff', border: 'none' }}>발송완료</button>
            <button style={{ ...btnStyle, background: 'var(--danger)', color: '#fff', border: 'none' }}>거래취소</button>
          </div>
        </div>
      </div>
    </Layout>
...
function StatusBadge({ status }: { status: string }) {
  const cfg: Record<string, { bg: string; color: string; label: string }> = {
    pending: { bg: 'var(--warning-light)', color: 'var(--warning)', label: '대기' },
    confirmed: { bg: 'var(--primary-light)', color: 'var(--primary)', label: '확인' },
    shipped: { bg: '#dbeafe', color: '#2563eb', label: '발송준비' },
    delivered: { bg: 'var(--success-light)', color: 'var(--success)', label: '발송완료' },
    cancelled: { bg: 'var(--danger-light)', color: 'var(--danger)', label: '취소' },
  }
