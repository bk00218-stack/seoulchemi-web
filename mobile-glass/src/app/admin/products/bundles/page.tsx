...
export default function BundlesPage() {
  ...
  const handleSubmit = async () => {
    if (!form.name || form.items.length === 0) {
      alert('Î¨∂Ïùå?ÅÌíàÎ™ÖÍ≥º Íµ¨ÏÑ±?ÅÌíà???ÖÎ†•?¥Ï£º?∏Ïöî')
      return
    }
  ...
  const handleDelete = async (bundle: Bundle) => {
    if (!confirm(`"${bundle.name}" Î¨∂Ïùå?ÅÌíà????†ú?òÏãúÍ≤†Ïäµ?àÍπå?`)) return
  ...
  return (
    <AdminLayout activeMenu="products">
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 600, margin: '0 0 8px' }}>Î¨∂Ïùå?ÅÌíà ?§Ï†ï</h1>
        <p style={{ color: 'var(--text-tertiary)', fontSize: '14px', margin: 0 }}>
          ?¨Îü¨ ?ÅÌíà??Î¨∂Ïñ¥???†Ïù∏ ?êÎß§?©Îãà??
        </p>
      </div>
              ...
              >
                ...
                <div style={{ display: 'flex', gap: '16px', fontSize: '14px' }}>
                  <span style={{ color: 'var(--text-tertiary)' }}>
                    ?ïÍ?: <span style={{ textDecoration: 'line-through' }}>{bundle.totalPrice.toLocaleString()}??/span>
                  </span>
                  {discount > 0 && (
                    <span style={{ color: '#dc2626' }}>
                      ?†Ïù∏: -{discount.toLocaleString()}??
                      {bundle.discountRate > 0 && ` (${bundle.discountRate}%)`}
                    </span>
                  )}
                  <span style={{ fontWeight: 600 }}>
                    ?êÎß§Í∞Ä: {finalPrice.toLocaleString()}??
                  </span>
                </div>
              </div>
            )
        ...
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ margin: '0 0 20px', fontSize: '18px' }}>
              {editingBundle ? 'Î¨∂Ïùå?ÅÌíà ?òÏ†ï' : 'Î¨∂Ïùå?ÅÌíà ?±Î°ù'}
            </h2>
