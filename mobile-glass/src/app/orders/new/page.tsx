...
// OlwsPro ?¤í???- ?˜ë‚˜???? ê°€?´ë° ê¸°ì?
// ?¸ë¡œ(?? = SPH: 0.00 ~ 15.00
// ê°€ë¡??? = CYL: ê°€?´ë° 000?ì„œ ?œì‘, ?‘ìª½?¼ë¡œ 400ê¹Œì?
// ?¼ìª½ = -Sph (ê·¼ì‹œ), ?¤ë¥¸ìª?= +Sph (?ì‹œ)
...
export default function NewOrderPage() {
  ...
  const [quantityActionSelection, setQuantityActionSelection] = useState<0 | 1 | 2>(0) // 0=ì¶”ê?, 1=?˜ì •, 2=ì·¨ì†Œ
  
  // ?¬ê³  ê·¸ë¦¬???°ì´??(SPH/CYL ì¡°í•©ë³??¬ê³ )
  ...
  const cylColsLeft = generateCylColsLeft()   // -4.00 ??0.00 (?¼ìª½, -Sph??
  const cylColsRight = generateCylColsRight() // 0.00 ??-4.00 (?¤ë¥¸ìª? +Sph??
  
  // ?„ì²´ ?? ?¼ìª½ CYL + ê°€?´ë° êµ¬ë¶„ + ?¤ë¥¸ìª?CYL
  const centerIndex = cylColsLeft.length // ê°€?´ë° ???¸ë±??
  ...
  useEffect(() => {
    const handleGlobalKeys = (e: globalThis.KeyboardEvent) => {
      if (e.key === 'F5') { 
        ...
        setGridFocus(null)
        setCellInputValue('')
        setSelectedBrandId(null)  // ?ˆëª© ì´ˆê¸°??
        setSelectedProductId(null)  // ?í’ˆ ì´ˆê¸°??
        ...
        if (gridFocus) { setGridFocus(null); setCellInputValue('') }
          ...
          setOrderItems([]); setMemo(''); // ì£¼ë¬¸ëª©ë¡ & ë©”ëª¨ ì´ˆê¸°??
  ...
  const handleProductListKeyDown = (e: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (e.key === 'ArrowDown') { e.preventDefault(); setProductFocusIndex(prev => Math.min(prev + 1, filteredProducts.length - 1)) }
      ...
      if (productFocusIndex >= 0 && productFocusIndex < filteredProducts.length) {
        ...
        setGridFocus({ sphIndex: 0, colIndex: cylColsLeft.length - 1 }) // ?¼ìª½ ??CYL 000)?ì„œ ?œì‘
        setCellInputValue('')
  ...
  const handleGridCellInput = useCallback((sphIndex: number, colIndex: number, quantity: number, forceMode?: 'add' | 'replace') => {
    // 0.5 ?¨ìœ„ë¡??¬ë¦¼ (?ˆê²½?Œì¦ˆ: 0.5 = ?œìª½, 1 = ?‘ìª½)
    const roundedQty = Math.ceil(quantity * 2) / 2 // 0.5 ?¨ìœ„ë¡??¬ë¦¼
    ...
    if (exists) {
      // ê¸°ì¡´ ?˜ëŸ‰???ˆê³  forceModeê°€ ?†ìœ¼ë©??ì—… ?„ìš°ê¸?
      if (!forceMode) {
        setQuantityActionSelection(0) // ê¸°ë³¸ê°? ì¶”ê? ë²„íŠ¼ ? íƒ
  ...
  const handleGridKeyDown = useCallback((e: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      setGridFocus(prev => {
        ...
        if (newCol === centerIndex) newCol++ // ê°€?´ë° ??ê±´ë„ˆ?°ê¸°
        // SPH 000 ?‰ì—?œëŠ” ?¤ë¥¸ìª??ì‹œ ?ì—­ ì§„ì… ë¶ˆê?
  ...
  const handleGridClick = useCallback((sphIndex: number, colIndex: number) => {
    if (!selectedProduct || !selectedStore) { alert('ê°€ë§¹ì ê³??í’ˆ??ë¨¼ì? ? íƒ?´ì£¼?¸ìš”.'); return }
  ...
  const handleSubmit = async () => {
    if (!selectedStore || orderItems.length === 0) { alert('ê°€ë§¹ì ê³??í’ˆ??? íƒ?´ì£¼?¸ìš”.'); return }
    setLoading(true)
    try {
      ...
      if (res.ok) {
        ...
        if (data.order?.id) {
          ...
          } catch (e) { console.error('ì¶œë ¥ ?¤íŒ¨:', e) }
        }
        // ?‘ìˆ˜ ?„ë£Œ ?ì—… ?œì‹œ
        setCompletedOrder({
  ...
  const renderCell = (sphIndex: number, colIndex: number) => {
    ...
    if (isDisabled) bg = '#d1d5db' // ë¹„í™œ?±í™”: ?Œìƒ‰
    else if (isCurrentRow || isCurrentCol) bg = '#c5dbc5' // ?¸ì´ì§€ ????
    if (!isDisabled && isCurrentRow && isCurrentCol) bg = '#a8c8a8' // êµì°¨????ì§„í•˜ê²?
  ...
  return (
    <Layout sidebarMenus={ORDER_SIDEBAR} activeNav="ì£¼ë¬¸">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, paddingBottom: 8, borderBottom: '2px solid #5d7a5d' }}>
        <h1 style={{ fontSize: 18, fontWeight: 700, margin: 0, color: 'var(--text-primary)' }}>?ë§¤?„í‘œ ?…ë ¥</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr 280px', gap: 4, height: 'calc(100vh - 110px)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3, background: 'var(--bg-secondary)', padding: 5, borderRadius: 3, overflow: 'hidden', fontSize: 13 }}>
          <section>
            <label style={{ fontWeight: 700, color: '#5d7a5d', display: 'flex', alignItems: 'center', gap: 4 }}>
              ?ª ?í˜¸ <span style={{ fontSize: 10, color: 'var(--text-tertiary)', fontWeight: 400 }}>[Esc]</span>
            </label>
            <input ref={storeInputRef} type="text" placeholder="ê±°ë˜ì²˜ëª…, ì½”ë“œ, ?„í™”ë²ˆí˜¸ë¡?ê²€??.." value={storeSearchText}
            ...
            {selectedStore && (
              <div style={{ marginTop: 6, padding: 10, background: 'linear-gradient(135deg, #eef4ee 0%, #d4e8d4 100%)', borderRadius: 8, fontSize: 12, lineHeight: 1.6, border: '1px solid #a8c8a8' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 10px' }}>
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>?šš <span style={{ color: '#333' }}>{selectedStore.deliveryContact || '-'}</span></span>
                </div>
          ...
          <section>
            <label style={{ fontWeight: 600, color: 'var(--text-primary)' }}>ì£¼ë¬¸ êµ¬ë¶„</label>
          ...
          <section>
            <label style={{ fontWeight: 600, color: 'var(--text-primary)' }}>?ˆëª© [F5]</label>
            <select ref={brandSelectRef} value={selectedBrandId || ''} onChange={e => { const bid = e.target.value ? parseInt(e.target.value) : null; setSelectedBrandId(bid); setSelectedProductId(null); if (bid) setTimeout(() => { setProductFocusIndex(0); productListRef.current?.focus() }, 50) }}
              ...
              <option value="">ë¸Œëœ??..</option>
              {brands.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
            </select>
          </section>
          <section style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <label style={{ fontWeight: 600, color: 'var(--text-primary)' }}>?í’ˆ [F6]</label>
            <div ref={productListRef} tabIndex={0} onKeyDown={handleProductListKeyDown} style={{ marginTop: 1, border: '1px solid #ccc', borderRadius: 2, background: 'var(--bg-primary)', flex: 1, overflow: 'auto', outline: 'none' }}>
              {filteredProducts.length === 0 ? <div style={{ padding: 4, textAlign: 'center', color: 'var(--text-tertiary)' }}>{selectedBrandId ? '?†ìŒ' : '? íƒ'}</div> : (
        ...
        <div ref={gridRef} tabIndex={0} onKeyDown={handleGridKeyDown}
          ...
          <div style={{ padding: '8px 12px', background: 'linear-gradient(135deg, #6b8e6b 0%, #4a6b4a 100%)', fontSize: 13, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontWeight: 700, color: '#fff', letterSpacing: '0.3px' }}>{selectedProduct ? `${selectedProduct.brand} - ${selectedProduct.name}` : '?í’ˆ ? íƒ'}</span>
            <span style={{ color: 'rgba(255,255,255,0.8)', fontSize: 11 }}>?â†’ CYL | ?‘â†“ SPH{focusedInfo ? <> | <strong style={{ color: '#fffacd' }}>?¬ê³ : {stockGrid[focusedInfo.sph.toFixed(2)]?.[focusedInfo.cyl.toFixed(2)] ?? '-'}</strong></> : ''}</span>
          </div>
          ...
          <div style={{ padding: '10px 14px', background: 'linear-gradient(135deg, #6b8e6b 0%, #4a6b4a 100%)', fontSize: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ color: 'rgba(255,255,255,0.9)' }}>{focusedInfo ? (() => {
              // CYL ?ŒëŸ¬???˜ì‚°: newSPH = SPH + CYL, newCYL = -CYL
            ...
            })() : <span style={{ color: 'rgba(255,255,255,0.7)' }}>?€ ? íƒ</span>}</span>
            <span style={{ color: focusedInfo?.isPlus ? '#ffcccb' : '#c5dbc5', fontWeight: 700, fontSize: 13 }}>{focusedInfo ? (focusedInfo.isPlus ? '?ì‹œ(+)' : 'ê·¼ì‹œ(-)') : ''}</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', background: 'var(--bg-secondary)', borderRadius: 3, overflow: 'hidden', fontSize: 13 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '50px minmax(60px, 1fr) 36px 36px 24px 52px', padding: '4px 6px', background: '#e0e0e0', fontWeight: 600, fontSize: 10, gap: '2px', alignItems: 'center' }}>
            ...
            <span style={{ textAlign: 'center' }}>?˜ëŸ‰</span>
            <span style={{ textAlign: 'right' }}>ê¸ˆì•¡</span>
          </div>
          <div style={{ flex: 1, overflow: 'auto' }}>
            {orderItems.length === 0 ? <div style={{ padding: 10, textAlign: 'center', color: 'var(--text-tertiary)' }}>?„ìˆ˜?œì—???˜ëŸ‰ ?…ë ¥</div> : (
          ...
          <div style={{ padding: '10px 12px', background: '#5d7a5d', color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 13 }}>
            <span>ì´?<strong>{totalQuantity}</strong>ê°?/span>
            <span style={{ fontSize: 16, fontWeight: 700 }}>{totalAmount.toLocaleString()}??/span>
          </div>
          <div style={{ padding: 6, display: 'flex', gap: 4 }}>
            <button onClick={() => setOrderItems([])} style={{ flex: 1, padding: 8, background: 'var(--bg-secondary)', border: '1px solid #ccc', borderRadius: 3, cursor: 'pointer', fontSize: 12 }}>ì´ˆê¸°??/button>
            <button onClick={handleSubmit} disabled={loading || !selectedStore || orderItems.length === 0} style={{ flex: 2, padding: 8, background: loading ? '#ccc' : '#4caf50', color: '#fff', border: 'none', borderRadius: 3, cursor: loading ? 'not-allowed' : 'pointer', fontSize: 13, fontWeight: 600 }}>?„ì†¡ [F2]</button>
          </div>
        </div>
      </div>
      ...
      {showCompleteModal && completedOrder && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ background: 'var(--bg-primary)', borderRadius: 16, padding: 32, width: 400, textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            <h2 style={{ fontSize: 24, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 8 }}>?‘ìˆ˜ ?„ë£Œ</h2>
            <p style={{ fontSize: 14, color: '#6b7280', marginBottom: 24 }}>ì£¼ë¬¸???•ìƒ?ìœ¼ë¡??‘ìˆ˜?˜ì—ˆ?µë‹ˆ??</p>
            ...
            <div style={{ background: '#f9fafb', borderRadius: 12, padding: 20, marginBottom: 24, textAlign: 'left' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12, fontSize: 14 }}>
                <span style={{ color: '#6b7280' }}>ì£¼ë¬¸? í˜•</span>
                <span style={{ fontWeight: 600, color: '#3b82f6' }}>{orderType}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12, fontSize: 14 }}>
                <span style={{ color: '#6b7280' }}>?í’ˆ??/span>
                <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{completedOrder.itemCount}ê±?/span>
              </div>
      ...
      {contextMenu && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9998 }} onClick={() => setContextMenu(null)}>
          <div style={{ position: 'absolute', top: contextMenu.y, left: contextMenu.x, background: 'var(--bg-primary)', border: '1px solid #ccc', borderRadius: 4, boxShadow: '0 2px 8px rgba(0,0,0,0.12)', overflow: 'hidden', width: 'fit-content' }} onClick={e => e.stopPropagation()}>
            <div onClick={() => handleEditQuantity(contextMenu.item)} style={{ padding: '4px 8px', cursor: 'pointer', borderBottom: '1px solid #eee', fontSize: 11, whiteSpace: 'nowrap' }} onMouseEnter={e => (e.target as HTMLElement).style.background = '#f5f5f5'} onMouseLeave={e => (e.target as HTMLElement).style.background = '#fff'}>?“ ?˜ëŸ‰</div>
            <div onClick={() => handleEditPrice(contextMenu.item)} style={{ padding: '4px 8px', cursor: 'pointer', borderBottom: '1px solid #eee', fontSize: 11, whiteSpace: 'nowrap' }} onMouseEnter={e => (e.target as HTMLElement).style.background = '#f5f5f5'} onMouseLeave={e => (e.target as HTMLElement).style.background = '#fff'}>?’° ê¸ˆì•¡</div>
            <div onClick={() => { removeItem(contextMenu.item.id); setContextMenu(null) }} style={{ padding: '4px 8px', cursor: 'pointer', fontSize: 11, color: '#e53935', whiteSpace: 'nowrap' }} onMouseEnter={e => (e.target as HTMLElement).style.background = '#ffebee'} onMouseLeave={e => (e.target as HTMLElement).style.background = '#fff'}>?—‘ï¸??? œ</div>
          </div>
        </div>
      )}
      ...
      {editModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ background: 'var(--bg-primary)', borderRadius: 12, padding: 24, width: 320, boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            <h3 style={{ margin: '0 0 16px', fontSize: 18, fontWeight: 600 }}>{editModal.type === 'quantity' ? '?˜ëŸ‰ ë³€ê²? : 'ê¸ˆì•¡ ë³€ê²?}</h3>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => { setEditModal(null); setEditValue('') }} style={{ flex: 1, padding: '10px', background: 'var(--bg-secondary)', border: '1px solid #ddd', borderRadius: 8, cursor: 'pointer', fontSize: 14 }}>ì·¨ì†Œ</button>
              <button onClick={handleEditConfirm} style={{ flex: 1, padding: '10px', background: '#5d7a5d', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 14, fontWeight: 600 }}>?•ì¸</button>
            </div>
          </div>
        </div>
      )}
