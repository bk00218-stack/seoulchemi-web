...
const SIDEBAR = [
  { title: '?ÑÍ≤∞??Ï£ºÎ¨∏', items: [
    { label: '?¨Î≤å Ï£ºÎ¨∏?¥Ïó≠', href: '/' },
    { label: 'RX Ï£ºÎ¨∏?¥Ïó≠', href: '/orders/rx' },
    { label: 'Í¥ÄÎ¶¨Ïûê Ï£ºÎ¨∏?±Î°ù', href: '/orders/new' },
    { label: 'Î™ÖÏÑ∏??Ï∂úÎ†•?¥Î†•', href: '/orders/print-history' },
  ]},
  { title: 'Ï∂úÍ≥†Í¥ÄÎ¶?, items: [
    { label: '?ÑÏ≤¥ Ï£ºÎ¨∏?¥Ïó≠', href: '/orders/all' },
    { label: 'Ï∂úÍ≥† ?ïÏù∏', href: '/orders/shipping' },
    { label: 'Ï∂úÍ≥† Î∞∞ÏÜ°ÏßÄ ?ïÎ≥¥', href: '/orders/delivery' },
  ]}
]
...
// OlwsPro ?§Ì???- ?òÎÇò???? Í∞Ä?¥Îç∞ Í∏∞Ï?
// ?∏Î°ú(?? = SPH: 0.00 ~ 15.00
// Í∞ÄÎ°??? = CYL: Í∞Ä?¥Îç∞ 000?êÏÑú ?úÏûë, ?ëÏ™Ω?ºÎ°ú 400ÍπåÏ?
// ?ºÏ™Ω = -Sph (Í∑ºÏãú), ?§Î•∏Ï™?= +Sph (?êÏãú)
...
// CYL ?? ?ºÏ™Ω?Ä 400??00, ?§Î•∏Ï™ΩÏ? 000??00
...
export default function NewOrderPage() {
  ...
  const [orderType, setOrderType] = useState<'?¨Î≤å' | 'Ï∞©ÏÉâ' | 'RX' | 'Í∏∞Ì?'>('?¨Î≤å')
  const [productFocusIndex, setProductFocusIndex] = useState<number>(-1)
  const [storeFocusIndex, setStoreFocusIndex] = useState<number>(-1)
  
  // Í∑∏Î¶¨?? colIndex = ?ÑÏ≤¥ ???∏Îç±??(0 = Îß??ºÏ™Ω CYL 400, Ï§ëÏïô = CYL 000, Îß??§Î•∏Ï™?= CYL 400)
  ...
  const cylColsLeft = generateCylColsLeft()   // -4.00 ??0.00 (?ºÏ™Ω, -Sph??
  const cylColsRight = generateCylColsRight() // 0.00 ??-4.00 (?§Î•∏Ï™? +Sph??
  
  // ?ÑÏ≤¥ ?? ?ºÏ™Ω CYL + Í∞Ä?¥Îç∞ Íµ¨Î∂Ñ + ?§Î•∏Ï™?CYL
  const centerIndex = cylColsLeft.length // Í∞Ä?¥Îç∞ ???∏Îç±??  const totalCols = cylColsLeft.length + 1 + cylColsRight.length
  ...
  // Í∑∏Î¶¨???¨Ïª§????Í∞Ä?¥Îç∞Î°??§ÌÅ¨Î°?  useEffect(() => {
    if (gridContainerRef.current && !gridFocus) {
      // Ï¥àÍ∏∞??Í∞Ä?¥Îç∞Î°??§ÌÅ¨Î°?      const container = gridContainerRef.current
      const scrollLeft = (centerIndex * 34) - (container.clientWidth / 2) + 50
      container.scrollLeft = Math.max(0, scrollLeft)
    }
  }, [selectedProductId])

  // ?ÅÌò∏ Í≤Ä??Í≤∞Í≥º ?§Î≥¥???¥Îèô ???§ÌÅ¨Î°?  useEffect(() => {
  ...
  // ?ÅÌíà Î™©Î°ù ?§Î≥¥???¥Îèô ???§ÌÅ¨Î°?  useEffect(() => {
  ...
  const handleProductListKeyDown = (e: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (e.key === 'ArrowDown') { e.preventDefault(); setProductFocusIndex(prev => Math.min(prev + 1, filteredProducts.length - 1)) }
      ...
      if (productFocusIndex >= 0 && productFocusIndex < filteredProducts.length) {
        ...
        setGridFocus({ sphIndex: 0, colIndex: cylColsLeft.length - 1 }) // ?ºÏ™Ω ??CYL 000)?êÏÑú ?úÏûë
        setCellInputValue('')
  ...
  const handleGridKeyDown = useCallback((e: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      setGridFocus(prev => {
        ...
        if (newCol === centerIndex) newCol++ // Í∞Ä?¥Îç∞ ??Í±¥ÎÑà?∞Í∏∞
  ...
  const handleGridClick = useCallback((sphIndex: number, colIndex: number) => {
    if (!selectedProduct || !selectedStore) { alert('Í∞ÄÎßπÏ†êÍ≥??ÅÌíà??Î®ºÏ? ?†ÌÉù?¥Ï£º?∏Ïöî.'); return }
  ...
  const handleSubmit = async () => {
    if (!selectedStore || orderItems.length === 0) { alert('Í∞ÄÎßπÏ†êÍ≥??ÅÌíà???†ÌÉù?¥Ï£º?∏Ïöî.'); return }
    setLoading(true)
    try {
      ...
      if (res.ok) {
        ...
        if (data.order?.id) {
          ...
          } catch (e) { console.error('Ï∂úÎ†• ?§Ìå®:', e) }
        }
        alert('Ï£ºÎ¨∏???±Î°ù?òÏóà?µÎãà??')
        router.push('/')
      } else alert('Ï£ºÎ¨∏ ?ùÏÑ± ?§Ìå®')
    } catch { alert('?§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.') }
    setLoading(false)
  }
  ...
  const renderCell = (sphIndex: number, colIndex: number) => {
    ...
    if (isCurrentRow || isCurrentCol) bg = '#ffcdd2' // ?ëÌÅ¨ ????    if (isCurrentRow && isCurrentCol) bg = '#ef9a9a' // ÍµêÏ∞®????ÏßÑÌïòÍ≤?    if (isFocused) bg = '#42a5f5'
    if (item) bg = '#4caf50'
    ...
    return (
      <td key={colIndex} onClick={() => handleGridClick(sphIndex, colIndex)}
        style={{ 
          ...
          cursor: 'pointer', width: 34, height: 24, fontSize: 13, 
  ...
  return (
    <Layout sidebarMenus={SIDEBAR} activeNav="Ï£ºÎ¨∏">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4, paddingBottom: 4, borderBottom: '2px solid #333' }}>
        <h1 style={{ fontSize: 16, fontWeight: 700, margin: 0 }}>?êÎß§?ÑÌëú ?ÖÎ†•</h1>
        <span style={{ fontSize: 16, color: '#666' }}>{new Date().toLocaleDateString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric', weekday: 'long' })}</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr 220px', gap: 4, height: 'calc(100vh - 110px)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3, background: '#f5f5f5', padding: 5, borderRadius: 3, overflow: 'hidden', fontSize: 11 }}>
          <section>
            <label style={{ fontWeight: 600 }}>?ÅÌò∏ [Esc]</label>
            <input ref={storeInputRef} type="text" placeholder="Í≤Ä??.." value={storeSearchText}
              ...
              style={{ width: '100%', padding: 4, border: '1px solid #ccc', borderRadius: 2, fontSize: 13, marginTop: 2 }} />
            {selectedStore && (
              <div style={{ marginTop: 3, padding: 5, background: '#e3f2fd', borderRadius: 2, fontSize: 16, lineHeight: 1.5 }}>
                <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 3 }}>{selectedStore.name}</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px 8px' }}>
                  <span>?éÔ∏è {selectedStore.phone || '-'}</span>
                  <span>?ì± {selectedStore.deliveryPhone || '-'}</span>
                  <span>?éØ {selectedStore.salesRepName || '-'}</span>
                  <span>?öö {selectedStore.deliveryContact || '-'}</span>
                </div>
                {selectedStore.address && (
                  <div style={{ marginTop: 2 }}>?ìç {selectedStore.address}</div>
                )}
                <div style={{ marginTop: 2, display: 'flex', gap: 8 }}>
                  <span style={{ color: (selectedStore.outstandingAmount || 0) > 0 ? '#c62828' : '#2e7d32', fontWeight: 600 }}>
                    ?í∞ {(selectedStore.outstandingAmount || 0).toLocaleString()}??                  </span>
                  {selectedStore.paymentTermDays ? (
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2 }}>
                      <span style={{ 
                        ...
                        <span style={{ fontSize: 13, fontWeight: 700, color: '#1d1d1f' }}>{selectedStore.paymentTermDays}</span>
                      </span>
                    </span>
                  ) : '-'}
                </div>
          ...
          <section>
            <label style={{ fontWeight: 600 }}>Ï£ºÎ¨∏ Íµ¨Î∂Ñ</label>
            <div style={{ display: 'flex', gap: 2, marginTop: 1 }}>
              {(['?¨Î≤å', 'Ï∞©ÏÉâ', 'RX', 'Í∏∞Ì?'] as const).map(t => (
                <label key={t} style={{ padding: '1px 4px', background: orderType === t ? '#1976d2' : '#fff', color: orderType === t ? '#fff' : '#333', border: '1px solid #ccc', borderRadius: 2, cursor: 'pointer', fontSize: 10 }}>
          ...
          <section>
            <label style={{ fontWeight: 600 }}>?àÎ™© [F5]</label>
            <select ref={brandSelectRef} value={selectedBrandId || ''} onChange={e => { const bid = e.target.value ? parseInt(e.target.value) : null; setSelectedBrandId(bid); setSelectedProductId(null); if (bid) setTimeout(() => { setProductFocusIndex(0); productListRef.current?.focus() }, 50) }}
              style={{ width: '100%', padding: 3, border: '1px solid #ccc', borderRadius: 2, fontSize: 13, marginTop: 1 }}>
              <option value="">Î∏åÎûú??..</option>
              {brands.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
            </select>
          </section>
          <section style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <label style={{ fontWeight: 600 }}>?ÅÌíà [F6]</label>
            <div ref={productListRef} tabIndex={0} onKeyDown={handleProductListKeyDown} style={{ marginTop: 1, border: '1px solid #ccc', borderRadius: 2, background: '#fff', flex: 1, overflow: 'auto', outline: 'none' }}>
              {filteredProducts.length === 0 ? <div style={{ padding: 4, textAlign: 'center', color: '#999' }}>{selectedBrandId ? '?ÜÏùå' : '?†ÌÉù'}</div> : (
                filteredProducts.map((p, i) => (
                  <div key={p.id} ref={el => { productItemRefs.current[i] = el }} onClick={() => { setSelectedProductId(p.id); setProductFocusIndex(i) }}
                    style={{ padding: '2px 3px', cursor: 'pointer', borderBottom: '1px solid #eee', background: selectedProductId === p.id ? '#e3f2fd' : productFocusIndex === i ? '#fff3e0' : '#fff', display: 'flex', justifyContent: 'space-between', fontSize: 10 }}>
        ...
        <div ref={gridRef} tabIndex={0} onKeyDown={handleGridKeyDown}
          ...
          <div style={{ padding: '2px 4px', background: '#e0e0e0', borderBottom: '1px solid #ccc', fontSize: 13, display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontWeight: 600 }}>{selectedProduct ? `${selectedProduct.brandName} - ${selectedProduct.name}` : '?ÅÌíà ?†ÌÉù'}</span>
            <span style={{ color: '#666' }}>?ê‚Üí CYL | ?ë‚Üì SPH | Í∞Ä?¥Îç∞=000</span>
          </div>
          
          <div ref={gridContainerRef} style={{ flex: 1, overflow: 'auto' }}>
            <table style={{ borderCollapse: 'collapse', fontSize: 16, fontFamily: 'monospace' }}>
              <thead>
          ...
          <div style={{ padding: '2px 4px', background: '#e0e0e0', borderTop: '1px solid #ccc', fontSize: 13, display: 'flex', justifyContent: 'space-between' }}>
            <span>{focusedInfo ? <>SPH: <strong>{focusedInfo.sph >= 0 ? '+' : ''}{focusedInfo.sph.toFixed(2)}</strong> | CYL: <strong>{focusedInfo.cyl.toFixed(2)}</strong></> : '?Ä ?†ÌÉù'}</span>
            <span style={{ color: focusedInfo?.isPlus ? '#e65100' : '#1565c0', fontWeight: 600 }}>{focusedInfo ? (focusedInfo.isPlus ? '?êÏãú(+)' : 'Í∑ºÏãú(-)') : ''}</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', background: '#f5f5f5', borderRadius: 3, overflow: 'hidden', fontSize: 9 }}>
          <div style={{ padding: '3px 4px', background: '#333', color: '#fff', fontWeight: 600, display: 'flex', justifyContent: 'space-between' }}>
            <span>Ï£ºÎ¨∏ Î™©Î°ù</span><span>{orderItems.length}Í±?/span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 36px 36px 28px 44px 16px', padding: '3px 4px', background: '#e0e0e0', fontWeight: 600, fontSize: 16, gap: '4px', alignItems: 'center' }}>
            <span style={{ whiteSpace: 'nowrap' }}>?ÅÌíà</span>
            <span style={{ textAlign: 'center' }}>SPH</span>
            <span style={{ textAlign: 'center' }}>CYL</span>
            <span style={{ textAlign: 'center' }}>?òÎüâ</span>
            <span style={{ textAlign: 'right' }}>Í∏àÏï°</span>
            <span></span>
          </div>
          <div style={{ flex: 1, overflow: 'auto' }}>
            {orderItems.length === 0 ? <div style={{ padding: 10, textAlign: 'center', color: '#999' }}>?ÑÏàò?úÏóê???òÎüâ ?ÖÎ†•</div> : (
              orderItems.map((item, i) => (
                <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '1fr 36px 36px 28px 44px 16px', padding: '3px 4px', borderBottom: '1px solid #ddd', background: i % 2 === 0 ? '#fff' : '#fafafa', alignItems: 'center', fontSize: 16, gap: '4px' }}>
                  ...
                  <button onClick={() => removeItem(item.id)} style={{ background: '#f44336', color: '#fff', border: 'none', borderRadius: '50%', width: 14, height: 14, cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>√ó</button>
                </div>
              ))
            )}
          </div>
          <div style={{ padding: 3, borderTop: '1px solid #ddd' }}>
            <input type="text" placeholder="Î©îÎ™®..." value={memo} onChange={e => setMemo(e.target.value)} style={{ width: '100%', padding: 2, border: '1px solid #ccc', borderRadius: 2, fontSize: 10 }} />
          </div>
          <div style={{ padding: '3px 4px', background: '#333', color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span>Ï¥?<strong>{totalQuantity}</strong>Í∞?/span>
            <span style={{ fontSize: 16, fontWeight: 700 }}>{totalAmount.toLocaleString()}??/span>
          </div>
          <div style={{ padding: 3, display: 'flex', gap: 3 }}>
            <button onClick={() => setOrderItems([])} style={{ flex: 1, padding: 4, background: '#f5f5f5', border: '1px solid #ccc', borderRadius: 2, cursor: 'pointer', fontSize: 9 }}>Ï¥àÍ∏∞??/button>
            <button onClick={handleSubmit} disabled={loading || !selectedStore || orderItems.length === 0} style={{ flex: 2, padding: 4, background: loading ? '#ccc' : '#4caf50', color: '#fff', border: 'none', borderRadius: 2, cursor: loading ? 'not-allowed' : 'pointer', fontSize: 13, fontWeight: 600 }}>?ÑÏÜ° [F2]</button>
          </div>
        </div>
      </div>
    </Layout>
