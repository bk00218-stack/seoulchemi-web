  ...
  const handleAdjust = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json()
        alert(data.error || '?¨Í≥† Ï°∞Ï†ï???§Ìå®?àÏäµ?àÎã§.')
      }
    } catch (error) {
  ...
  return (
    <AdminLayout activeMenu="products">
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 600, margin: '0 0 8px' }}>?¨Í≥† Í¥ÄÎ¶?/h1>
        <p style={{ color: 'var(--text-tertiary)', fontSize: '14px', margin: 0 }}>?ÅÌíàÎ≥??¨Í≥† ?ÑÌô©???ïÏù∏?òÍ≥† ?ÖÏ∂úÍ≥†Î? Ï≤òÎ¶¨?©Îãà??</p>
      </div>

      {/* ?µÍ≥Ñ Ïπ¥Îìú */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
        <div style={{ background: 'var(--bg-primary)', borderRadius: '12px', padding: '20px' }}>
          <div style={{ fontSize: '13px', color: 'var(--text-tertiary)', marginBottom: '4px' }}>?ÑÏ≤¥ ?ÅÌíà</div>
          <div style={{ fontSize: '28px', fontWeight: 600 }}>{stats.totalProducts}</div>
        </div>
        <div style={{ background: 'var(--bg-primary)', borderRadius: '12px', padding: '20px' }}>
          <div style={{ fontSize: '13px', color: 'var(--text-tertiary)', marginBottom: '4px' }}>Ï¥??¨Í≥†</div>
          <div style={{ fontSize: '28px', fontWeight: 600 }}>{stats.totalStock.toLocaleString()}</div>
        </div>
        <div style={{ background: 'var(--bg-primary)', borderRadius: '12px', padding: '20px' }}>
          <div style={{ fontSize: '13px', color: '#f59e0b', marginBottom: '4px' }}>?Ä?¨Í≥†</div>
          <div style={{ fontSize: '28px', fontWeight: 600, color: '#f59e0b' }}>{stats.lowStock}</div>
        </div>
        <div style={{ background: 'var(--bg-primary)', borderRadius: '12px', padding: '20px' }}>
          <div style={{ fontSize: '13px', color: '#ef4444', marginBottom: '4px' }}>?àÏ†à</div>
      ...
      <div style={{ background: 'var(--bg-primary)', borderRadius: '12px', padding: '16px', marginBottom: '16px', display: 'flex', gap: '12px' }}>
        ...
        >
          <option value="all">?ÑÏ≤¥ Î∏åÎûú??/option>
          {brands.map(brand => (
      ...
      <div style={{ background: 'var(--bg-primary)', borderRadius: '12px', overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-tertiary)' }}>?¨Í≥† ?∞Ïù¥?∞Í? ?ÜÏäµ?àÎã§.</div>
        ) : (
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                        ...
                        <th style={{ padding: '10px 8px', textAlign: 'left', fontWeight: 500, color: '#6b7280' }}>?µÏÖò</th>
                        <th style={{ padding: '10px 8px', textAlign: 'left', fontWeight: 500, color: '#6b7280' }}>Î∞îÏΩî??/th>
                        <th style={{ padding: '10px 8px', textAlign: 'left', fontWeight: 500, color: '#6b7280' }}>?ÑÏπò</th>
                        <th style={{ padding: '10px 8px', textAlign: 'right', fontWeight: 500, color: '#6b7280' }}>?¨Í≥†</th>
                        <th style={{ padding: '10px 8px', textAlign: 'center', fontWeight: 500, color: '#6b7280' }}>Ï°∞Ï†ï</th>
                      </tr>
                    </thead>
                    <tbody>
      ...
      {showAdjustModal && adjustTarget && (
        <div style={{
          ...
          <div style={{ background: 'var(--bg-primary)', borderRadius: '16px', padding: '24px', width: '400px' }}>
            <h2 style={{ fontSize: '18px', fontWeight: 600, marginBottom: '8px' }}>?¨Í≥† Ï°∞Ï†ï</h2>
            ...
            <div style={{ marginBottom: '16px' }}>
              <div style={{ fontSize: '14px', marginBottom: '8px' }}>?ÑÏû¨ ?¨Í≥†: <strong>{adjustTarget.option.stock}</strong></div>
            </div>

            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontSize: '14px', fontWeight: 500, marginBottom: '6px' }}>Ï°∞Ï†ï ?†Ìòï</label>
              <div style={{ display: 'flex', gap: '8px' }}>
                {[
                  { value: 'in', label: '?ÖÍ≥† (+)' },
                  { value: 'out', label: 'Ï∂úÍ≥† (-)' },
                  { value: 'adjust', label: 'ÏßÅÏ†ë ?§Ï†ï' }
                ].map(opt => (
                  <button
                    key={opt.value}
            ...
            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontSize: '14px', fontWeight: 500, marginBottom: '6px' }}>
                {adjustType === 'adjust' ? 'Î≥ÄÍ≤ΩÌï† ?¨Í≥† ?òÎüâ' : 'Ï°∞Ï†ï ?òÎüâ'}
              </label>
              <input
                type="number"
                value={adjustQty}
            ...
            <div style={{ marginBottom: '24px' }}>
              <label style={{ display: 'block', fontSize: '14px', fontWeight: 500, marginBottom: '6px' }}>Î©îÎ™®</label>
              <input
                type="text"
                value={adjustMemo}
                onChange={e => setAdjustMemo(e.target.value)}
                placeholder="Ï°∞Ï†ï ?¨Ïú† ?ÖÎ†•"
                style={{ width: '100%', padding: '10px 12px', borderRadius: '6px', border: '1px solid var(--border-color)', fontSize: '14px' }}
              />
            </div>
