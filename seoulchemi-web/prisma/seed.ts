...
async function main() {
  ...
  for (const store of storesData.stores) {
    await prisma.store.create({
      data: {
        ...
        isActive: store.status === '사용'
      }
    })
  }

  // 브랜드 추출 및 입력
  const brandNames = [...new Set(productsData.products.map((p: any) => p.brand))]
  ...
  for (let i = 0; i < brandNames.length; i++) {
    const brand = await prisma.brand.create({
      data: {
        name: brandNames[i] as string,
        displayOrder: i,
        isActive: true
      }
    })
    brandMap[brandNames[i] as string] = brand.id
  }
